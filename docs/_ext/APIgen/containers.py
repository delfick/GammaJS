...
from APIgen.utility import cleanstr
from APIgen.regexes import regexes
...
class Everything(Info):
    ...
    def finish(self):
        self.moduleNames         = sorted(self[MODULES].keys(), key=lambda x: x.lower())
    ...
    def _add(self, state, name, kls, lookIn, desc):
        lookAt = self[lookIn]
        if name in lookAt:
            if kls is None and not isinstance(lookAt, (list, tuple)):
                kls = lookAt[name]
            else:
                log.warn("WARNING: %s %s was redefined" % (desc, name))
        else:
            if isinstance(lookAt, (list, tuple)):
                lookAt.append(kls)
            else:
