        ...
        if len(parts) > 2:
            ...
            if kls in classes:
                ...
                if kls not in classes:
                    raise Exception(
                        f"Trying and failing to find kls '{kls}' from '{'.'.join(parts)}'"
                    )
                kls = classes[kls]
            ...
            if inherited:
                target = f"{parts[0]}.{inherited}.{last}"
...
class PropertyRole(BaseRole):
    pass


def setup(app):
    app.add_crossref_type(
        directivename="entity", rolename="entity", indextemplate="pair: %s; entity",
    )
    ...
    app.add_crossref_type(
        directivename="api", rolename="api", indextemplate="single: %s",
    )
    ...
    app.add_crossref_type(
        directivename="metho", rolename="metho",
    )

    app.add_crossref_type(
        directivename="prop", rolename="prop",
    )
    ...
    def apply(self):
        for node in self.document.traverse(nodes.block_quote):
            if len(node.children) == 1 and isinstance(
                node.children[0], self.suppress_blockquote_child_nodes
            ):
                node.replace_self(node.children[0])


class GmaHTMLTranslator(HTMLTranslator):
    """
